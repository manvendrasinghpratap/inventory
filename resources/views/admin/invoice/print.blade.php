    ...
    * { -webkit-print-color-adjust: exact; }
	...
	<div id="yourdiv">
		...
		<article>
			<h1>Recipient</h1>
			<address >
				<p>{{ @$bank_name }}<br>{{ @$branch_name }}<br>{{ @$state_name }}</p>
			</address>
			<table class="meta">
				<tr>
			...
			<table class="inventory">
                ...
                @php( $i=0)
					...
					<tr>
						<td><span>{{ @$row[productDetails]['name'] }}</span></td>
						<td><span>{{ @$row['name'] }}</span></td>
						<td><span>{{ @$row['rate'] }}</span></td>
						<td><span>{{@$quantity[$i]}}</span></td>
						<td><span>{{@$ratebyquantities[$i]}}</span></td>
					</tr>
					@php ($i++)
					@endforeach
				</tbody>
			...
			<table class="balance">
				...
				<tr>
					...
					<td> @php($nil = 'Nill')@if($shippingcharges>0){{ @$shippingcharges }} @else {{ $nil}} @endif</td>
				</tr>
				<tr>
